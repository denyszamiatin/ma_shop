...
class MarksApi(Resource):
    def get(self):
        marks = MarkSchema.query.all()
        return marks_schema.dump(marks)

    def post(self):
        json_data = request.json
        try:
            mark = marks_schema.load(json_data, session=db.session)
        except ValidationError as error:
            return {'message': str(error)}, 422
        try:
            db.session.add(mark)
...
class MarkApi(Resource):
    def get(self, uuid):
        mark = Mark.query.filter_by(uuid=uuid).first()
        if not mark:
    ...
    def delete(self, uuid):
        mark = Mark.query.filter_by(uuid=uuid).first()
        if not mark:
            return {"massage": "Mark not found"}, 404
